...
[HarmonyPatch]
internal static class EndlessModeHpScalingPatch
{
	[CompilerGenerated]
	...
	{
		if (!PlusState.IsEndlessModeActive() || !GameReflection.IsEnemyCreature(__instance) || !AppliedTracker.MarkEndlessScaled(__instance))
		{
			return;
		}
		decimal endlessHpMultiplier = GameReflection.GetEndlessHpMultiplier();
		if (!(endlessHpMultiplier <= 1.0m))
		{
